...
type PipeConfigSource = 'project' | 'settings';

type ActivePipeConfiguration = {
  serverPipe: string;
  clientPipe: string;
...
type RuntimeButtonId = 'run' | 'stop' | 'rebuild';
type RuntimeSignalKind = 'status' | 'alert';
...
function handleIncomingPipeCommand(command: PipeCommandPayload): void {
...
function handleRuntimeLogCommand(command: PipeCommandPayload): boolean {
...
function logRuntimeMessage(level: RuntimeLogLevel, rawMessage: string): void {
...
async function sendRuntimeCommand(command: string, args?: string[]): Promise<void> {
  if (!activePipeBridge) {
...
class QBookViewProvider implements vscode.WebviewViewProvider {
  ...
  private async handleToggleHidden(message: BridgeMessage): Promise<void> {
  ...
  private async handleUpdateTitle(message: BridgeMessage): Promise<void> {
    const targetPage = this.resolveNodePageIdentifier(message);
    if (!targetPage) {
      vscode.window.showWarningMessage('Keine Page ausgewählt.');
      return;
  ...
  private async handleUpdateFormat(message: BridgeMessage): Promise<void> {
    const targetPage = this.resolveNodePageIdentifier(message);
    if (!targetPage) {
      vscode.window.showWarningMessage('Keine Page ausgewählt.');
      return;
  ...
  private async handleRenamePage(message: BridgeMessage): Promise<void> {
    ...
    try {
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          await this.updatePageMetadataFile(
            newPage,
            (doc) => {
              doc.Name = newPage;
  ...
  private async handleDeletePage(message: BridgeMessage): Promise<void> {
    ...
    const confirmation = await vscode.window.showWarningMessage(
      `Möchten Sie die Page '${targetPage}' inklusive aller Dateien löschen?`,
      { modal: true },
      'Löschen'
    );
  ...
  private async handleReorderPages(message: BridgeMessage): Promise<void> {
  ...
  private async handleCreatePage(): Promise<void> {
    if (!this.lastBookRoot) {
      vscode.window.showWarningMessage('Kein qBook-Projekt geladen.');
      return;
    }
  ...
  private async handleImportPage(): Promise<void> {
    if (!this.lastBookRoot) {
      vscode.window.showWarningMessage('Kein qBook-Projekt geladen.');
      return;
    }
  ...
  private async handleAddSubcode(message: BridgeMessage): Promise<void> {
    const targetPage = this.normalizePageName(this.resolveNodePageIdentifier(message));
    if (!targetPage) {
      vscode.window.showWarningMessage('Keine Page ausgewählt.');
      return;
  ...
  private async handleRenameSubnode(message: BridgeMessage): Promise<void> {
  ...
  private async handleDeleteSubnode(message: BridgeMessage): Promise<void> {
    const pageName = this.normalizePageName(message.page);
    if (!pageName) {
      vscode.window.showWarningMessage('Keine Page für die Datei ausgewählt.');
      return;
  ...
  private normalizePageName(value?: string | null): string | undefined {
  ...
  private normalizeCodeName(value?: string | null): string | undefined {
    const trimmed = value?.trim();
    if (!trimmed) {
      return undefined;
    }
  ...
  private normalizeSubnodeFileName(pageName: string, fileName?: string | null): string | undefined {
  ...
  private extractSubnodeCodeName(pageName: string, fileName: string): string | undefined {
  ...
  private isProtectedSubnodeFile(pageName: string, fileName: string): boolean {
    const codeName = this.extractSubnodeCodeName(pageName, fileName);
    return Boolean(codeName && codeName.toLowerCase() === 'qpage');
  }

  private async renamePageFolder(oldPage: string, newPage: string): Promise<void> {
  ...
  private async renamePageFiles(oldPage: string, newPage: string): Promise<void> {
  ...
  private async renameNamespaceReferences(oldPage: string, newPage: string): Promise<void> {
    if (!this.lastBookRoot) {
      return;
    }
  ...
  private async renamePageInBookFile(oldPage: string, newPage: string): Promise<void> {
    if (!this.lastBookRoot) {
  ...
  private async writeBookPageOrder(order: string[]): Promise<void> {
    if (!this.lastBookRoot) {
      throw new Error('Kein qBook-Projekt geladen.');
    }
  ...
  private async ensurePageOrderEntry(pageName: string): Promise<void> {
    if (!this.lastBookRoot) {
      throw new Error('Kein qBook-Projekt geladen.');
    }
  ...
  private async removePageOrderEntry(pageName: string): Promise<void> {
    if (!this.lastBookRoot) {
      throw new Error('Kein qBook-Projekt geladen.');
    }
  ...
  private async updateProgramClass(): Promise<void> {
    if (!this.lastBookRoot) {
      return;
    }
  ...
  private async getOrderedPageNames(): Promise<string[]> {
    if (!this.lastBookRoot) {
      return [];
    }
  ...
  private generateProgramClassContent(pageNames: string[]): string {
...
`;
  ...
  private async copyDirectoryRecursive(source: vscode.Uri, target: vscode.Uri): Promise<void> {
  ...
  private getBookFileUri(): vscode.Uri {
    if (!this.lastBookRoot) {
      throw new Error('Kein qBook-Projekt geladen.');
    }
  ...
  private async updatePageMetadataFile(
    pageName: string,
  ...
  private generateQPageTemplate(pageName: string): string {
...
`;
  ...
  private generateOPageTemplate(pageName: string): string {
    const document = {
      Name: pageName,
      Text: pageName,
      OrderIndex: 0,
  ...
  private generateSubcodeTemplate(pageName: string, codeName: string): string {
...
`;
  ...
  private rewriteFileReferences(document: OPageDocument, context: { oldPage: string; newPage: string }): void {
  ...
  private renameSubnodeReferences(
  ...
  private removeSubnodeReferences(document: OPageDocument, pageName: string, fileName: string): void {
    const refFields: Array<keyof OPageDocument> = ['Includes', 'CodeOrder'];
    for (const field of refFields) {
      const value = document[field];
      if (!Array.isArray(value)) {
  ...
  private matchesSubnodeReference(entry: unknown, pageName: string, fileName: string): boolean {
    if (typeof entry !== 'string') {
      return false;
    }
  ...
  private canonicalizeSubnodeReference(pageName: string, entry: string): string | undefined {
  ...
  private getPageDirectory(pageName: string): vscode.Uri {
    if (!this.lastBookRoot) {
      throw new Error('Kein qBook-Projekt geladen.');
    }
  ...
  private reportMetadataError(action: string, error: unknown): void {
    const details = error instanceof Error ? error.message : String(error);
    vscode.window.showErrorMessage(`Page-Metadaten (${action}) konnten nicht gespeichert werden: ${details}`);
  }

  private resolveNodePageIdentifier(message: BridgeMessage): string | undefined {
  ...
  private async readPageFolder(bookRoot: vscode.Uri, pageName: string): Promise<BookTreeNode> {
  ...
  private relativePathFromUri(uri?: vscode.Uri): string | undefined {
  ...
  private getFormStateForSelection(): FormState | undefined {
  ...
  private async discoverPageFolders(bookRoot: vscode.Uri): Promise<string[]> {
    const pagesDir = vscode.Uri.joinPath(bookRoot, 'Pages');
  ...
  private async findBookFile(): Promise<vscode.Uri | undefined> {
  ...
  private isPrimaryCodeFile(fileName: string): boolean {
    const lower = fileName.toLowerCase();
    return lower.endsWith('.qpage.cs') || lower.endsWith('.opage.cs');
  }

  private getDisplayName(pageName: string, fileName: string): string {
    if (!fileName) {
      return '';
    }
  ...
  private async safeReadDirectory(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
    try {
