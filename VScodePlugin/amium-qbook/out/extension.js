...
class QBookViewProvider {
    ...
    constructor(context) {
        ...
        this.context.subscriptions.push(vscode.debug.onDidStartDebugSession((session) => {
            ...
            const runtimeChannel = getRuntimeLogChannel();
            runtimeChannel?.info(`Debug active-session changed: ${session ? `${session.name}#${session.id} type=${session.type}` : 'none'}`);
        }));
    }
    ...
    async handleStartDebugging() {
        ...
        runtimeChannel?.info(`Attach run #${runId} context: lastBookRoot=${this.lastBookRoot?.fsPath ?? 'n/a'} workspaceFolders=${(vscode.workspace.workspaceFolders ?? []).map((folder) => folder.uri.fsPath).join(' | ') || 'none'}`);
        ...
        if (launchConfigSelection) {
            const selectedConfig = launchConfigSelection.configuration;
            runtimeChannel?.info(`Attach run #${runId} launch-config-selected=${launchConfigSelection.configurationName} file=${launchConfigSelection.sourcePath}`);
            runtimeChannel?.info(`Attach run #${runId} launch-defaults=${JSON.stringify({
        ...
        const startedFromLaunch = await this.startDebuggingFromProjectLaunchConfig(debugWorkspace, launchConfigSelection, runId, startedAt);
    ...
    logDebugLandscape(scope) {
        ...
        runtimeChannel.info(`Debug landscape (${scope}): active=${active ? `${active.name}#${active.id} type=${active.type}` : 'none'} totalSessions=${sessions.length}`);
        if (sessions.length > 0) {
            runtimeChannel.info(`Debug sessions (${scope}): ${sessions
                .map((session) => `${session.name}#${session.id} type=${session.type} managed=${this.isManagedDebugSession(session)}`)
                .join(' | ')}`);
        }
        const breakpoints = vscode.debug.breakpoints;
        const sourceBreakpoints = breakpoints.filter((breakpoint) => breakpoint instanceof vscode.SourceBreakpoint);
        const functionBreakpoints = breakpoints.filter((breakpoint) => breakpoint instanceof vscode.FunctionBreakpoint);
        const otherBreakpoints = breakpoints.length - sourceBreakpoints.length - functionBreakpoints.length;
        runtimeChannel.info(`Breakpoints (${scope}): total=${breakpoints.length} source=${sourceBreakpoints.length} function=${functionBreakpoints.length} other=${otherBreakpoints}`);
    ...
    async startDebuggingFromProjectLaunchConfig(workspaceFolder, launchSelection, runId, startedAt) {
        const runtimeChannel = getRuntimeLogChannel();
        if (!workspaceFolder) {
        ...
        const launchConfiguration = launchSelection.configuration;
        const hasProcessId = typeof launchConfiguration.processId === 'number' ||
            (typeof launchConfiguration.processId === 'string' && launchConfiguration.processId.trim().length > 0);
        const hasProcessName = typeof launchConfiguration.processName === 'string' && launchConfiguration.processName.trim().length > 0;
        runtimeChannel?.info(`Resolved launch config payload (run=${runId}): ${JSON.stringify({
        ...
        if (!hasProcessId && !hasProcessName) {
            runtimeChannel?.error(`Launch-config attach aborted (run=${runId}): configuration '${launchSelection.configurationName}' has neither processId nor processName`);
            return false;
        }
        runtimeChannel?.info(`Starting resolved launch configuration '${launchSelection.configurationName}' (run=${runId}) from ${workspaceFolder.uri.fsPath}`);
        try {
            ...
            if (started) {
                runtimeChannel?.info(`==== qBook Attach Run #${runId} ATTACHED totalElapsed=${Date.now() - startedAt}ms sessionId=${vscode.debug.activeDebugSession?.id ?? 'n/a'} ====`);
            }
            return started;
        }
        catch (error) {
    ...
    async resolveProjectAttachConfiguration(rootUri) {
        ...
        for (const source of launchSources) {
            ...
            const normalized = {
                ...this.cloneJsonValue(selected),
                name: selected.name ?? 'qBook Attach',
                request: 'attach',
                type: selected.type ?? 'coreclr',
    ...
    async readLaunchConfigurationSources(rootUri) {
        if (!rootUri) {
            return [];
        }
    ...
    pickAttachConfiguration(configurations, preferredNames) {
        ...
        for (const preferredName of preferredNames) {
            const match = attachConfigs.find((entry) => typeof entry.name === 'string' && entry.name.toLowerCase() === preferredName.toLowerCase());
            if (match) {
                return match;
            }
        }
    ...
    buildAttachAttemptConfigs(processId, processName, preferredArchitecture, launchDefaults = {}) {
        const result = [];
        const adapterTypes = ['coreclr', 'clr'];
    ...
    async tryStartDebuggingViaLaunchConfig(config, runId) {
        const workspaceFolder = this.resolveDebugWorkspaceFolder();
        if (!workspaceFolder) {
            const runtimeChannelNoWorkspace = getRuntimeLogChannel();
            runtimeChannelNoWorkspace?.error(`Fallback attach aborted${runId ? ` (run=${runId})` : ''}: no workspace folder resolved`);
            return false;
        }
        const runtimeChannel = getRuntimeLogChannel();
        runtimeChannel?.info(`Direct attach failed${runId ? ` (run=${runId})` : ''}, fallback to .vscode/launch.json in ${workspaceFolder.uri.fsPath}`);
        try {
            ...
            runtimeChannel?.info(`Fallback launch.json startDebugging returned ${started}${runId ? ` (run=${runId})` : ''}`);
            return started;
        }
        catch (error) {
            const details = error instanceof Error ? error.message : String(error);
    ...
    async writeAttachLaunchConfig(rootUri, config) {
        ...
        const attachEntry = {
            ...this.cloneJsonValue(existingAttach),
            name: 'qBook Attach',
            type: config.type,
            request: 'attach',
            justMyCode: false,
    ...
    async readLaunchAttachDefaults(rootUri) {
        if (!rootUri) {
            return {};
        }
        const launchUri = vscode.Uri.joinPath(rootUri, '.vscode', 'launch.json');
    ...
    extractAttachOptionDefaults(configuration) {
        const result = {};
        const keys = [
            'justMyCode',
            'requireExactSource',
    ...
    logDebugProviderState(channel) {
        ...
        channel.info(`Debug provider state: csharp(installed=${Boolean(csharp)}, active=${Boolean(csharp?.isActive)}) csdevkit(installed=${Boolean(devkit)}, active=${Boolean(devkit?.isActive)})`);
    }
    logDebugSessionEvent(eventType, session) {
        const runtimeChannel = getRuntimeLogChannel();
        if (!runtimeChannel) {
            return;
        }
        const configuration = session.configuration;
        runtimeChannel.info(`Debug session ${eventType}: name=${session.name} id=${session.id} type=${session.type} managed=${this.isManagedDebugSession(session)} config=${JSON.stringify({
