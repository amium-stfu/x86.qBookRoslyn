...
namespace Main
{
	...
	public partial class FormCodeEditor : System.Windows.Forms.Form {
        ...
        //oPage _StartupPage = null;
        //public FormCodeEditor(oPage page)
        //	: this()
        //{
        //	_StartupPage = page;
        //      }
		...
		class SyntaxEditorDocumentWindow : DocumentWindow
		{
            ...
            public void SetCsScriptEditorCaretLineCharacter(int line, int character, int endLine = -1, int endCharacter = -1)
            {
				if (line < 0)
					return;
        ...
        private SyntaxEditorDocumentWindow ShowCodeEditorDocument(oPage page, TreeNode treeNode, string language = "cs", string subCodeKey=null, bool readOnly=false) {
            ...
            //    var ref1 = projectAssembly.AssemblyReferences.AddFrom(System.IO.Path.GetFullPath("qbScriptLib.dll"));
            ...
            Qb.CsScriptAssembly.AssemblyReferences.ItemAdded -= AssemblyReferences_Changed;
            Qb.CsScriptAssembly.AssemblyReferences.ItemAdded += AssemblyReferences_Changed;
            Qb.CsScriptAssembly.AssemblyReferences.ItemRemoved -= AssemblyReferences_Changed;
            Qb.CsScriptAssembly.AssemblyReferences.ItemRemoved += AssemblyReferences_Changed;
            ...
            if (page.CsCodeSourceFileKey != null)
            {
                ...
                if (Qb.CsScriptAssembly.SourceFiles.Contains(page.CsCodeSourceFileKey))
                    Qb.CsScriptAssembly.SourceFiles.Remove(page.CsCodeSourceFileKey);
            }
			if (language == "py") {
				syntaxEditor.Document.Language = Qb.PyScriptLanguage;
                syntaxEditor.Document.TabSize = 4;
				syntaxEditor.Document.AutoConvertTabsToSpaces = true;
			}
			else { 
				syntaxEditor.Document.Language = Qb.CsScriptLanguage;
			}

            page.CsCodeSourceFileKey = Qb.CsScriptAssembly.SourceFiles.LastOrDefault()?.Key; //update the key

            var fooFiles = Qb.CsScriptAssembly.SourceFiles;
        ...
        private void RefreshReferenceListCallback(object stateInfo)
        {
            ...
            foreach (var assemblyRef in Qb.CsScriptAssembly.AssemblyReferences.ToArray())
        ...
        int idxCsCode = 3;
        int idxPyCode = 4;
        void PopulatePageTreeView(bool selectFirstPage = false)
        {
			try
			{
				treeViewCodePages.Nodes.Clear();
				var pageList = Qb.Book.Main.Objects.OfType<oPage>();
				...
				foreach (var page in pageList)
				{
					...
					{
						var pageNode = new TreeNode(page.Name, idxPage, idxPage);
						pageNode.Tag = page;
        ...
        private TreeNode FindTreeNode(TreeView treeView, oPage page)
        {
        ...
        private TreeNode FindTreeNode(TreeNode node, oPage page)
        {
		...
		public void SelectPage(oPage page)
		{
		...
		oPage SelectedPage = null;
        private void treeViewCodePages_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                var page = e.Node.Tag as oPage;
                if (page != null)
                {
                    SetStatusText("ready");
                    SelectedPage = (e.Node.Tag as oPage);
                    if (e.Node.ImageIndex == idxPage)
                    {
                        if (SelectedPage != null)
                            Qb.SelectedPage = SelectedPage; //show page in mainform
			...
			//else if (e.Node.Parent?.Tag is oPage)
			//{
			//	SelectedPage = (e.Node.Parent?.Tag as oPage);
			//	//toolStripTextBoxPageName.Text = SelectedPage.Name.Replace("\r\n", "\n").Trim('\n').Replace("\n", "\r\n");
   ...
   //         }
			//else
   //         {
   //             if (e.Node.Parent?.Tag is oPage)
			//	{
			//		page = e.Node.Parent.Tag as oPage;
...
//}";
        ...
        void CombineCodeFragments(oPage page, string headerText, string footerText)
        {
			if (page == null)
				return;
		...
		internal async void ReInit()
		{
            ...
            foreach (var page in Qb.Book.Main.Objects)
            {
                //var codeEditorTemp =
                page.MySyntaxEditor = new ActiproSoftware.UI.WinForms.Controls.SyntaxEditor.SyntaxEditor();
                page.MySyntaxEditor.Document.Language = Qb.CsScriptLanguage;
                ...
                page.CsCodeSourceFileKey = Qb.CsScriptAssembly.SourceFiles.Last().Key;
            }
        }
        ...
        async Task RebuildCodeAllAsync()
        {
            ...
            //qb.Widget.Chart chart = new qb.Widget.Chart();
            ...
            try
            {
                ...
                //    Qb.Book.Serialize();
                //    Qb.Book.Modified = false;
                //}

                try
                {
                    await Qb.CsScriptRebuildAll(); //BuildCsScriptAll();

                    if (false)
                    {
                        if (Qb.LastBuildErrors != null)
                        {
                            SetStatusText("#ERR: rebuild took " + sw.ElapsedMilliseconds.ToString() + "ms", Color.Red);
                            toolWindowOutput.Select();
                            OutputWriteLine("#EX: " + Qb.LastBuildErrors);
                        }
                    }
                    ...
                    this.EnsureInvokeAsync(() =>
                    {
                        ...
                        for (int errorLineNr = 0; errorLineNr<errorLines.Length; errorLineNr++)
						{
							...
							if (m.Success)
							{
								...
								while (lineNr >= 0)
								{
									if (sourceCodeOffsetRegex.IsMatch(Qb.LastCsScriptBuildCodeLines[lineNr]))
									{
										Match m1 = sourceCodeOffsetRegex.Match(Qb.LastCsScriptBuildCodeLines[lineNr]);
										if (m1.Success) 
										{
											string source = m1.Groups["source"].Value;
                                            int offset = int.Parse(m1.Groups["line"].Value);
                                            errorLines[errorLineNr] = $"<{source}>({linesUp+offset},{col+1}): " + error;
											break;
										}
									}
									linesUp++;
            ...
            {
                sw.Stop();
                this.EnsureInvokeAsync(() => this.Cursor = Cursors.Default);
        ...
        private void textBoxOutput_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m.Success)
                {
					...
					foreach (var p in Qb.Book.Main.Objects.OfType<oPage>())
					{
                    ...
                    if (page != null)
					{
                        ...
                        docWindow = ShowCodeEditorDocument(page as oPage, myTreeNode, "cs", subCodeKey, false);
                        docWindow.Activate();
                        docWindow.SetCsScriptEditorCaretLineCharacter(lineNr, col);

                    }
                    else
					{
						int origLineNr = lineNr;
                        MessageBox.Show("??? Could not open page-editor\r\n\r\nBad code-line is:\r\n" + Qb.LastCsScriptBuildCodeLines[origLineNr], $"ERROR at {origLineNr+1},{col+1}");
                    }


        //            string[] lines = Qb.LastCsScriptBuildCode.Replace("\r", "").Split('\n');
        //            if (lines.Length > lineNr)
        //            {
        //                //scan backwards to previus "//=== class '"
        //                int pageLineNr = lineNr;
        //                int pageLineOffset = 0;
        //                while (pageLineNr > 0 && !lines[pageLineNr].StartsWith("//=== Page"))
        //                {
        //                    pageLineNr--;
        //                    pageLineOffset++;
        //                }
        //                if (pageLineNr > 0)
        //                {
        //                    m = pageNameRegex.Match(lines[pageLineNr]);
        //                    if (m.Success)
        //                    {
        //                        bool isSubPage = m.Groups["sub"].Value.Length > 0;
        //                        string pageFullName = m.Groups["name"].Value;
								//string pageName = null;
								//if (isSubPage)
								//{
        //                            int lastDot = pageFullName.LastIndexOf('.');
								//	if (lastDot >= 0) {
								//		pageName = pageFullName.Substring(0, lastDot);
        //                                subCodeKey = pageFullName.Substring(lastDot+1);
								//	}
								//	else
								//	{
        ...
        //                            pageName = pageFullName;
        //                        }
        //                        var page = Qb.Book.Main.Objects.FirstOrDefault(p => p.Name == pageName);
        //                        if (page != null)
        //                        {
								//	if (subCodeKey != null)
								//		pageLineOffset = pageLineOffset - 1;
								//	else
								//		pageLineOffset = pageLineOffset - 1 - 1;// - 2;

								//	TreeNode myTreeNode = null; //TODO: find matching treenode

								//	SyntaxEditorDocumentWindow docWindow = null;
								//	if (errorText.Contains(": error CS") || radioButtonLangCS.Checked)
								//	{
        //                                //skip using-lines 
        //                                int usingCount = 0;
        //                                foreach (string l in page.CsCode.Replace("\r", "").Split('\n'))
								//		{
								//			//if (l.TrimStart().StartsWith("using"))
								//				usingCount++;
								//			if (!l.TrimStart().StartsWith("//") && l.Contains("@class_" + page.Name))
								//				break;
								//		}
								//		docWindow = ShowCodeEditorDocument(page as oPage, myTreeNode, "cs", subCodeKey, false);
								//		docWindow.Activate();
								//		docWindow.SetCsScriptEditorCaretLineCharacter(pageLineOffset + usingCount -1, col);
								//	}
								//	else
								//	{
								//		docWindow = ShowCodeEditorDocument(page as oPage, myTreeNode, "py", subCodeKey, false);
								//		docWindow.Activate();
								//		docWindow.SetCsScriptEditorCaretLineCharacter(pageLineOffset, col);
								//	}
								//	return;
        //                        }
        //                    }
        ...
        //                MessageBox.Show("??? Could not open page-editor\r\n\r\nBad code is:\r\n" + lines[lineNr], $"ERROR at {lineNr},{col}");
//                    }
                }
            }
            catch (Exception ex)
        ...
        void RenameSymbol()
        {
            ...
            foreach (var findItem in findList)
            {
                var page = Qb.Book.Main.Objects.FirstOrDefault(o => o.CsCodeSourceFileKey == findItem.SyntaxEditorKey);
                string pageName = "?";
                int lineNr = -1;
        ...
        void GotoDefinition2()
        {
            ...
            if (resolverResultSet != null)
            {
                foreach (var resolverResult in resolverResultSet.Results)
                {
                    ...
                    {
                        TreeNode myTreeNode = null; //TODO: find matching treenode
                        var page = Qb.Book.Main.Objects.FirstOrDefault(o => o.CsCodeSourceFileKey == sourceFileLocation.Key);
                        if (page != null)
                        {
                            //var editForm = EditObjectForm.Edit(0, 0, page);
        ...
        private void treeViewCodePages_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
        {
			...
			if (e.Node != null && e.Node.Tag is oPage)
			{
				var page = e.Node.Tag as oPage;
				if (page != null)
				{
					if (e.Node.ImageIndex == idxPage)
					{
						...
						if (index == -1)
                            ...
                            if (Qb.Book.Main.Objects.Count(i => i.FullName == newFullName) > 0)
						{
							MessageBox.Show("Duplicate Page-Key");
							e.CancelEdit = true;
							e.Node.BeginEdit();
							return;
						}
						else
						{
							...
							if (SelectedPage != null && e.Label.Trim() != origNodeText)
							{
                                ...
                                for (int i=0; i<csCodeLines.Length; i++)
								{
									...
									{
										Match m = classNameRegex.Match(line);
                                        int nameStartIndex = m.Groups["name"].Index;
                                        int nameEndIndex = nameStartIndex + m.Groups["name"].Length;
        ...
        private void deleteCodeClassToolStripMenuItem_Click(object sender, EventArgs e)
        {
			...
			if (dr == DialogResult.OK)
			{
				if (_lastClickedNode.Tag is oPage)
				{
					if (_lastClickedNode.ImageIndex == idxCsCode)
					{
						oPage page = _lastClickedNode.Tag as oPage;
		...
		private void toolStripButtonHeaderFooter_Click(object sender, EventArgs e)
		{
            ...
            Qb.UpdateProjectAssemblyQbRoot();
        }
		...
		private void treeViewCodePages_KeyDown(object sender, KeyEventArgs e)
		{
			...
			{
                if (treeViewCodePages.SelectedNode != null && treeViewCodePages.SelectedNode.Tag is oPage)
                {
                    //page-node
                ...
                if (treeViewCodePages.SelectedNode != null && treeViewCodePages.SelectedNode.Parent?.Tag is oPage)
				{
					//subcode-node
					treeViewCodePages.LabelEdit = true;
					treeViewCodePages.SelectedNode.BeginEdit();
        ...
        private void FormCodeEditor_KeyDown(object sender, KeyEventArgs e)
		{
            ...
            {
                //find next
				if (toolWindowFindResults.Active)
				{
        ...
        private void DoFindTextInCode()
        {
            ...
            var dr = InputDialog.ShowDialog("FIND IN CODE", "Find Text:", ref text);
            if (dr == DialogResult.OK)
            {
                FindTextInCode(text);
            }
        ...
        void FindTextInCode(string findTerm)
        {
            List<FindResult> findList = new List<FindResult>();
            foreach (var page in Qb.Book.Main.Objects.OfType<oPage>())
            {
                string[] codeLines = page.CsCode.GetLines(); // .Replace("\r", "").Split('\n');
                int lineNr = 0;
				var findRegex = new Regex(findTerm, RegexOptions.IgnoreCase);
                foreach (string line in codeLines)
                {
                    lineNr++;
			...
			catch (Exception ex)
			{
				//if (ex is IronPython.Runtime.UnboundNameException)
				{
					...
					if (true)
					{
						...
						//SyntaxEditorDocumentWindow docWindow = null;
						//if (radioButtonLangPython.Checked)
						//    docWindow = ShowCodeEditorDocument(page as oPage, "py", subCodeKey, false);
						//else
						//    docWindow = ShowCodeEditorDocument(page as oPage, "cs", subCodeKey, false);
		...
		{
			//Qb.ShowOpenQbookFileDialog(sender);
		}
		...
		private void toolStripDropDownButtonOpen_DropDownOpening(object sender, EventArgs e)
		{
            ...
            var mruItems = Qb.MruFilesManager.MruItems; //.GetMruStringList();
        ...
        private void MruItemClicked(object sender, EventArgs e)
        {
            ...
            if (!System.IO.File.Exists(clickedItem.Name))
            {
                ...
                {
                    Qb.MruFilesManager.Remove(clickedItem.Name);
                    Properties.Settings.Default.MruFileList = Qb.MruFilesManager.GetMruCsvString();
            ...
            else
            {
                ...
                Qb.OpenQbookAsync(clickedItem.Name);
				if (Qb.Book != null)
				{
					try
					{
						Application.OpenForms[0].Bounds = Qb.Book.Bounds;
					}
					catch { }
				}
                ReInit();
        ...
        private void MruItemDeleteIconClicked(object sender, ToolStripMenuItemEx.DeleteItemClickedEventArgs e)
        {
            ...
            if (filename != null)
            {
                Qb.MruFilesManager.Remove(filename);
                Properties.Settings.Default.MruFileList = Qb.MruFilesManager.GetMruCsvString();
                Properties.Settings.Default.Save();
		...
		{
            string usingsCode = Qb.GetUsingsCode();
            string csScriptCode = Qb.CsScriptCombineCode();
			string fullCode = usingsCode + "\r\n" + Qb.ProgramMainCode + csScriptCode;
		...
		private void GotoFindItem(ListViewItem item, bool activateDocWindow = false)
        {
            ...
            if (findItem != null)
            {
                var docWindow = ShowCodeEditorDocument(findItem.Page as oPage, null, subCodeKey: findItem.SubCodeKey);
        ...
        private void attachDebuggerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (System.IO.File.Exists(debuggerFilename))
            {
                ...
                if (!string.IsNullOrEmpty(Qb.LastScriptDllFilename))
                    args.Add("\"" + Qb.LastScriptDllFilename + "\"");
        ...
        private void tESTLoadScriptDllToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (false)
            {
                ...
                //Qb.ActiveCsAssembly = (System.Reflection.Assembly)csScript.GetType().Assembly; //hope this works?! 
                //Qb.CsScript_Init();
                //Qb.LastBuildResult = null;
                ...
                try
                {
                    ...
                    //SCAN         QB.Automation.Signalgenerator.SignalGenList.Clear();
            ...
            if (true)
            {
                for (int i = 0; i < 1; i++) //TEST only
                {
                    ...
                    Qb.LastScriptDllFilename = finalDllFilename;
                    //Qb.Book = book;
