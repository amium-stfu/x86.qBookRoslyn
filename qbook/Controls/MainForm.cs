...
{string.Join("\n", referenceItems)}
        ...
        private static string GetExePath(bool released)
        {
            ...
            string projectDir = Directory.GetParent(binDir)!.FullName;         // ...\qBookStudio\
  
            if (!released)
            {
                string solutionDir = Directory.GetParent(Directory.GetParent(projectDir)!.FullName).FullName;
                QB.Logger.Debug("solutionDir:  " + solutionDir);
                return Path.Combine(solutionDir, @"qbookCode\bin\Debug\net8.0-windows\qbookCode.exe");
            }
            else
            {
                return Path.Combine(projectDir, @"qbookCode\qbookCode.exe");
            }
        ...
        private void newEditorToolStripMenuItem_Click(object sender, EventArgs e)
        {
           // BookBuilder.CreateBook("T:\\qbooksave\\Demo", "TestBook");
            ...
            if (Debugger.IsAttached)
            {
                exePath = GetExePath(released: false);
            }
            else
            {
                exePath = GetExePath(released: true);
            }

            string args =  $"\"{path}\" " + $"\"{PipeNames.Server}\" " + $"\"{PipeNames.Client}\"";

            QB.Logger.Debug("Starting Code-Editor in: " + exePath);
