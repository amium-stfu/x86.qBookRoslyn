...
namespace qbook.ScintillaEditor
{
    ...
    public class BookTreeView : System.Windows.Forms.TreeView
    {
        ...
        {
            if (e.Data.GetDataPresent(typeof(BookNode)))
            {
                Point pt = PointToClient(new Point(e.X, e.Y));
                System.Windows.Forms.TreeNode targetNode = GetNodeAt(pt);
                BookNode draggedNode = (BookNode)e.Data.GetData(typeof(BookNode));
            ...
            if (e.Button == MouseButtons.Left)
            {
                if (e.Node is BookNode)
                {
                    ...
                    if (View != null)
                    {
                        ...
                        View.SetTarget(SelectedCodeNode.Editor);
                    }
        ...
        private async void renamePageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(name))
                {
                    ...
                    Core.ActualMain.Objects.Remove(originPage);
                    Core.ActualMain.Objects.Add(newPage);
        ...
        private BookNode GetNodeByFilename(string name)
        {
            Debug.WriteLine($"========");
            foreach (BookNode node in Nodes[0].Nodes)
            {
