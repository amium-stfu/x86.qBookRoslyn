...
{
    internal static class RoslynFoldingHelper
    {
            ...
            string name)
        {
            ...
            var span = node.Span;
            var startLine = editor.LineFromPosition(span.Start);
            var endLine = editor.LineFromPosition(span.End);

            // ❌ Entfernen: Verschiebung auf { - Zeile
        ...
        #region Save & Restore
        public static void SaveFolding(Scintilla editor)
        {
            ...
            foreach (var line in editor.Lines)
            {
                ...
                {
                    bool collapsed = !line.Expanded;

                    // 🧠 Zusatz: Wenn Header expanded ist, aber nächste Zeile '{' collapsed
                    if (!collapsed && line.Index + 1 < editor.Lines.Count)
                    {
                        var nextLine = editor.Lines[line.Index + 1];
                        var nextText = nextLine.Text.Trim();
                        if (nextText == "{" && nextLine.FoldLevelFlags.HasFlag(FoldLevelFlags.Header) && !nextLine.Expanded)
                        {
                            collapsed = true;
                        }
                    }
                    ...
                    if (collapsed)
                    {
                        string key = GetFoldKey(line);
                        _collapsedHeaders.Add(key);
                      //  Debug.WriteLine($"[FoldSave] {key}");
                    }
                }
            }
        }
        ...
        public static void RestoreFolding(Scintilla editor)
        {
            if (_collapsedHeaders.Count == 0)
                return;
            ...
            foreach (var line in editor.Lines)
            {
                ...
                {
                    string key = GetFoldKey(line);
                    if (_collapsedHeaders.Contains(key) && line.Expanded)
                    {
                        line.ToggleFold();
                     //   Debug.WriteLine($"[FoldRestore] Folded '{key}'");
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Liefert einen eindeutigen Key pro Foldbereich:
        /// - Zeilentext falls sinnvoll
        /// - Bei '{' oder leeren Zeilen Zeilenindex als Fallback
        /// </summary>
        private static string GetFoldKey(Line line)
        {
