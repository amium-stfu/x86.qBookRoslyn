...
namespace qbookCode.Roslyn
{
    public static class RosylnSignatureHelper
    {
        ...
        public static BookNode ActiveNode { get; set; } = null;
        public static async Task<bool> TrySignatureHelpAsync(bool updateOnly = false)
        {
            try
            {
                if (!ActiveNode.Editor.Target.Active)
                {
                ...
                var doc = Core.Roslyn.GetDocumentByFilename(ActiveNode.Editor.Target.Filename);
