...
namespace qbookCode
{
    internal static class Core
    {
        ...
        { 
            ...
            File.WriteAllText(Path.Combine(bookFolder, Core.ThisBook.Filename), link);
        ...
        public static async Task SaveProjectAsync(string bookFolder = @"T:\qSave")
        {
            ...
            string codeDir = Path.Combine(bookFolder, "Pages");
            ...
            var project = new qBookDefinition
            {
                ...
                Directory = Program.ActualBookPath,
            ...
            string path = Path.Combine(bookFolder, "Program.cs");
            File.WriteAllText(path, code);

            code = await Core.Roslyn.GetDocumentTextAsync("GlobalUsing.cs");
            path = Path.Combine(bookFolder, "GlobalUsing.cs");
            File.WriteAllText(path, code);

            string bookJson = JsonConvert.SerializeObject(project, Newtonsoft.Json.Formatting.Indented);
            File.WriteAllText(Path.Combine(bookFolder, "Book.json"), bookJson);

            Debug.WriteLine("Project saved to " + bookFolder);

            string programFile = Path.Combine(bookFolder, "Program.cs");
