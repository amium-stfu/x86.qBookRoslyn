...
namespace QB.Net
{
    public partial class Modbus
    {
        ...
        {
            static List<Client> ModbusClients = new List<Client>();

            internal static void ResetClients()
            {
                foreach (var c in ModbusClients)
                    c.Destroy();//SCAN //TODO
                ModbusClients.Clear();
            }

            public Client(string name, string uri = "tcp://127.0.0.1:502") : base(name)
            {
                Uri = uri;
                ModbusClients.Add(this);
            ...
            public void Connect(string socket)
            {
                //Close();
                this.socket = socket;
                modbusClient = new _ModbusClient(Name, socket);
                ...
                lock (this)
                {
                    ...
                    try
                    {
                        ...
                        //    if (lockTaken)
                        {
                            ...
                            int dur = 500;
                            if (modbusClient != null)
                                response = modbusClient.Transceive(command, ref dur);
        ...
        class _ModbusClient
        {
            ...
            bool tcp = false;
            public string uri = "127.0.0.1:502";


            static List<_ModbusClient> ModbusClients = new List<_ModbusClient>();
            ...
            public _ModbusClient(string text, string socket)
            {
                ModbusClients.Add(this);
                Text = text;
